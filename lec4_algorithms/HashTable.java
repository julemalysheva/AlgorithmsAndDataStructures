...
public class HashTable<K, V> {
    ...
    //Метод hash в хеш-таблице выполняет вычисление хеш-кода ключа, который используется для определения индекса
    // элемента в массиве table.
    //
    //Хеш-код - это целочисленное значение, которое вычисляется на основе содержимого ключа. Цель хеш-функции состоит
    // в том, чтобы максимально равномерно распределять значения ключей по всему массиву table. Хорошая хеш-функция
    // должна давать разные хеш-коды для разных ключей и избегать коллизий (когда два разных ключа имеют одинаковый хеш-код).
    //
    //Существует множество способов реализации хеш-функций. Одним из наиболее распространенных способов является
    // использование простейшего метода, который называется методом деления по модулю.
    //
    //В этом методе вычисление хеш-кода происходит следующим образом:
    //
    //На вход хеш-функции подается ключ.
    //Ключ преобразуется в целочисленное значение.
    //Целочисленное значение делится на размер массива table (обычно простым числом) с помощью операции модуля (%).
    //Остаток от деления и будет являться хеш-кодом для данного ключа.
    //Например, если размер массива table равен 10, а ключ - это строка "hello", то можно вычислить хеш-код следующим образом:
    //int hashCode = "hello".hashCode(); // преобразуем строку в целочисленное значение
    //int index = Math.abs(hashCode % 10); // вычисляем индекс в массиве table
    //Здесь метод hashCode используется для преобразования строки в целочисленное значение.
    // Метод Math.abs используется для получения положительного значения остатка от деления.
    //
    //Таким образом, метод hash в хеш-таблице выполняет вычисление хеш-кода ключа с использованием
    // выбранной хеш-функции. Результатом работы метода является индекс элемента в массиве table,
    // где данный ключ будет храниться или где можно будет найти элемент по данному ключу.
}
